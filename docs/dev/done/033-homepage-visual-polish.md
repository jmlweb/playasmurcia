...
- [x] Fix two consecutive white sections: change region highlights section from `bg-white` to `bg-sand-50` to restore alternating background pattern
- [x] Add section heading to region highlights (e.g., "Lo que hace unica a la Costa Calida") with the established label + h2 pattern
- [x] Strengthen homepage municipality cards: increase padding, add visual weight (icon, prominent beach count, or left accent)
- [x] Standardize primary CTA button color: use `bg-ocean-500` consistently for all primary buttons (currently mixed `bg-ocean-500` and `bg-ocean-600`)
