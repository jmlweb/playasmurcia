...
export const MenuItem = ({ title, basePath, data }: Props) => (
  <Item>
    <Trigger className="group flex select-none items-center justify-between gap-1 px-3 py-5 text-[15px] font-medium leading-none text-gray-200 transition-colors motion-safe:hover:text-sky-300 focus:shadow-md">
      {title}
      <LuChevronDown />
    </Trigger>
    <Content
        ...
        {data.map((dataItem) => (
          <li key={dataItem.slug}>
            <Link href={`${basePath}/${dataItem.slug}`} legacyBehavior passHref>
              <NavLink className="group flex select-none items-center py-3 text-[15px] font-medium leading-none text-gray-500 no-underline transition-colors motion-safe:hover:text-gray-700">
                {dataItem.name}{' '}
