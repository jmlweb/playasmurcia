...
import { applyFiltersAndSort, filterBeaches, sortBeaches } from "./beach-filters"
...
const beaches: Array<Beach> = [
  makeBeach({ name: "Cala Abierta", municipality: 0, sea: 0, services: [0, 1], activities: [0], tags: [0, 2], length: 300 }),
  makeBeach({ name: "Playa de Bolnuevo", municipality: 3, sea: 0, services: [0], activities: [0, 1], tags: [1], length: 1200, occupancyLevel: "high" }),
  makeBeach({ name: "Las Salinas", municipality: 4, sea: 1, services: [0, 2], activities: [2], tags: [2, 3], length: 800, occupancyLevel: "low" }),
  makeBeach({ name: "Playa Honda", municipality: 6, sea: 1, services: [], activities: [], tags: [], length: 500, occupancyLevel: "medium" }),
  makeBeach({ name: "Calblanque", municipality: 0, sea: 0, services: [1, 3], activities: [0, 3], length: 2000, occupancyLevel: "low" }),
]
...
describe("filterBeaches", () => {
  ...
  it("ignores empty filter arrays", () => {
    const result = filterBeaches(beaches, { municipality: [], sea: [], services: [] })
...
describe("applyFiltersAndSort", () => {
  it("filters and sorts in one step", () => {
    ...
    expect(result[0].length).toBeGreaterThanOrEqual(result[result.length - 1].length ?? 0)
  })
})
