...
describe("NearbyCarousel", () => {
  ...
  it("renders the heading", () => {
    render(
      <NearbyCarousel
        items={[{ beach: makeBeach(), municipality: mockMunicipality, slug: "playa-test" }]}
      />,
    )
    expect(screen.getByRole("heading", { name: "Playas cercanas" })).toBeTruthy()
  })

  it("renders a link for each nearby beach", () => {
    const items = [
      { beach: makeBeach({ code: "001", name: "Playa A" }), municipality: mockMunicipality, slug: "playa-a" },
      { beach: makeBeach({ code: "002", name: "Playa B" }), municipality: mockMunicipality, slug: "playa-b" },
    ]
  ...
  it("renders the default image when no pictures are available", () => {
    render(
      <NearbyCarousel
        items={[{ beach: makeBeach({ name: "Sin Foto" }), municipality: mockMunicipality, slug: "sin-foto" }]}
      />,
    )
